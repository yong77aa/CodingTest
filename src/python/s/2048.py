        ...
        for j in range(size):
            ...
            for i in range(1, size):
                if board[i][j]:
                    ...
                    if board[i][top] == 0:  # 이동하려는 자리의 값이 0이면
                        board[i][top] = temp  # 이전 자리의 값을 넣음
                    elif board[i][top] == temp:  # 이동하려는 자리의 값이 이전 자리의 값과 같은 경우
                        board[i][top] = temp * 2  # 이전 자리의 값에서 곱하기 2한 값을 넣음
                        top += 1
                    else:  # 이동하려는 자리의 값이 이전 자리의 값과 다르고 0이 아닌 경우
                        top += 1
                        board[i][j] = temp  # 원래 자리에다가 다시 넣음
